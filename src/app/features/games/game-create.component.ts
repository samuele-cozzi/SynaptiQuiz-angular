    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mt-8 space-y-8 divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <div class="space-y-6 sm:space-y-5">
            ...
            <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 dark:sm:border-gray-700 sm:pt-5">
              ...
              <div class="mt-1 sm:mt-0 sm:col-span-2">
                <input type="text" [(ngModel)]="gameData.name" class="max-w-lg block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:max-w-xs sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white p-2">
              </div>
            </div>
             ...
             <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 dark:sm:border-gray-700 sm:pt-5">
              ...
              <div class="mt-1 sm:mt-0 sm:col-span-2">
                 <select [(ngModel)]="gameData.language" (change)="onLanguageChange()" class="max-w-lg block focus:ring-indigo-500 focus:border-indigo-500 w-full shadow-sm sm:max-w-xs sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white p-2">
        ...
        <div class="pt-8 space-y-6 sm:space-y-5">
             <div>
               <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">Select Players</h3>
               <p class="mt-1 max-w-2xl text-sm text-gray-500">Select at least one player.</p>
            </div>
            
            <div class="sm:flex sm:items-center">
                 <input type="text" [(ngModel)]="playerSearch" placeholder="Filter players..." class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white p-2 max-w-xs">
            </div>

            <!-- Players Grid -->
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 h-64 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-md p-4">
               @for (player of filteredPlayers(); track player.uid) {
                  <div class="relative flex items-center p-2 rounded hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer" 
                       (click)="togglePlayer(player)"
                       [class.bg-indigo-50]="isSelectedPlayer(player.uid)"
                       [class.dark:bg-indigo-900]="isSelectedPlayer(player.uid)">
                     <div class="flex items-center h-5">
                       <input type="checkbox" [checked]="isSelectedPlayer(player.uid)" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded pointer-events-none">
                     </div>
                     <div class="ml-3 text-sm">
                       <label class="font-medium text-gray-700 dark:text-gray-200">{{ player.username }}</label>
                     </div>
                  </div>
               }
            </div>
        ...
        <div class="pt-8 space-y-6 sm:space-y-5">
             <div>
               <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">Select Questions</h3>
             ...
             <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
               <input type="text" [(ngModel)]="questionSearch" placeholder="Search text..." class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white p-2">
               <select [(ngModel)]="questionDifficulty" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white p-2">
            ...
            <!-- Questions Grid -->
            <div class="grid grid-cols-1 gap-4 h-96 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-md p-4">
               @for (q of filteredQuestions(); track q.id) {
                  <div class="relative flex items-start p-2 rounded hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer" 
                       (click)="toggleQuestion(q)"
                       [class.bg-indigo-50]="isSelectedQuestion(q.id)"
                       [class.dark:bg-indigo-900]="isSelectedQuestion(q.id)">
                     <div class="flex items-center h-5 mt-1">
                       <input type="checkbox" [checked]="isSelectedQuestion(q.id)" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded pointer-events-none">
                     </div>
                     <div class="ml-3 text-sm">
             ...
             <p class="text-sm" [class.text-red-600]="!isValidDistribution()" [class.text-green-600]="isValidDistribution()">
                Selected: {{ selectedQuestions().length }}
                @if (selectedPlayers().length > 0) {
                    (Per player: {{ selectedQuestions().length / selectedPlayers().length | number:'1.0-2' }})
                }
             </p>
             @if (!isValidDistribution() && selectedPlayers().length > 0) {
                 <p class="text-sm text-red-600">Total questions must be divisible by number of players.</p>
             }
        </div>
        
        <div class="pt-5">
          <div class="flex justify-end">
            <a routerLink="/games" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:bg-gray-600 dark:text-white dark:border-gray-500">
               {{ 'COMMON.CANCEL' | translate }}
            </a>
            <button (click)="createGame()" [disabled]="!canCreate()"
...
export class GameCreateComponent implements OnInit {
    ...
    filteredPlayers() {
        return this.allPlayers().filter(p => !this.playerSearch || p.username.toLowerCase().includes(this.playerSearch.toLowerCase()));
    }

    filteredQuestions() {
        return this.allQuestions().filter(q => {
            const matchesText = !this.questionSearch || q.text.toLowerCase().includes(this.questionSearch.toLowerCase());
            const matchesDiff = this.questionDifficulty === null || q.difficulty === this.questionDifficulty;

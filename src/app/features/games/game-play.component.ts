      ...
      <div class="md:flex md:items-center md:justify-between mb-6">
        <div>
          ...
          <p class="text-sm text-gray-500">
            Status: {{ g.status }} | Questions: {{ g.playerAnswers.length }}/{{
              g.questions.length
            }}
          </p>
        </div>
          ...
          <div class="flex">
            ...
            <div class="ml-3">
              <p class="text-sm text-indigo-700 dark:text-indigo-200">
                Current Turn:
                <span class="font-bold">{{ getPlayerName(g.currentTurnPlayerId) }}</span>
                @if (isMyTurn(g)) { (YOU) }
              </p>
            </div>
          </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg overflow-hidden relative">
          ...
          <div class="border-t border-gray-200 dark:border-gray-700 px-4 py-5 sm:p-6">
            ...
            <div class="flex items-center space-x-3 mb-4">
              <button
                (click)="useExternalHelp(g)"
                [disabled]="(getMyGamePlayer(g)?.externalHelps || 0) <= 0"
              ...
              <button
                (click)="useFiftyFifty(g)"
                [disabled]="(getMyGamePlayer(g)?.fiftyFifty || 0) <= 0"
                class="inline-flex items-center px-3 py-1.5 border border-gray-300 rounded-md text-sm bg-white dark:bg-gray-700 hover:bg-gray-50 disabled:opacity-50"
              >
            ...
            <div class="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2">
              ...
              <button
                (click)="submitAnswer(g, q.id, $index)"
                ...
                [class.opacity-50]="
                  (!isMyTurn(g) && !isAdmin()) || hiddenAnswerIndexes().includes($index)
                "
...
export class GamePlayComponent implements OnInit, OnDestroy {
  ...
  getTopicImageUrl(topicId: string): string {
    ...
    return (
      topic?.imageUrl ||
      'data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%20100%20100%22%3E%3Crect%20width%3D%22100%22%20height%3D%22100%22%20fill%3D%22%23cccccc%22%2F%3E%3C%2Fsvg%3E'
    );
  }
  ...
  async useExternalHelp(g: Game) {
    ...
    const updatedPlayers = g.players.map((p) =>
      p.uid === me.uid ? { ...p, externalHelps: (p.externalHelps || 0) - 1 } : p
    );
  ...
  async useFiftyFifty(g: Game) {
    ...
    const arr = q.answers.map((a, idx) => ({
      idx,
      plaus: a.plausibility || 0,
      correct: !!a.correct,
    }));
    ...
    const updatedPlayers = g.players.map((p) =>
      p.uid === me.uid ? { ...p, fiftyFifty: (p.fiftyFifty || 0) - 1 } : p
    );
    await this.gameService.updateGame(g.id, { players: updatedPlayers });
  }
  ...
  async useSwitch(g: Game) {
    ...
    const updatedPlayers = g.players.map((p) =>
      p.uid === me.uid ? { ...p, switches: (p.switches || 0) - 1 } : p
    );
  ...
  isMyTurn(g: Game) {
    return (
      g.status === 'in_progress' && g.currentTurnPlayerId === this.authService.currentUser()?.uid
    );
  }

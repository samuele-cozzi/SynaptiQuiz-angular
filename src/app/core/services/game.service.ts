...
export class GameService {
  ...
  async submitAnswer(gameId: string, game: Game, questionId: string, answerIndex: number) {
    ...
    if (
      !currentUser ||
      (game.currentTurnPlayerId !== currentUser.uid &&
    ...
    const points = isCorrect
      ? pointsMap[question.difficulty] || 10
      : -(pointsMap[question.difficulty] || 10) / 2;
    ...
    const currentPlayerIndex = game.players.findIndex((p) => p.uid === game.currentTurnPlayerId);
        ...
        try {
          ...
          if (playerSnap.exists()) {
            ...
            await this.playerService.updatePlayer(gamePlayer.uid, {
              ...
              gamesWon: isWinner
                ? (currentPlayerData.gamesWon || 0) + 1
